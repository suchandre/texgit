			...
			if (isNotNull(record.getDeclaredInnerRecords()) && !record.getDeclaredInnerRecords().isEmpty()) {
				...
				for (String id : record.getDeclaredInnerRecords()) {
					...
					try {
						...
						throw new IllegalStateException(format("Erro ao tentar ler o registro \"%s\".", innerRec.getName()), e);
					}
				}
			}
		}
			...
			if (isRepitable(id)) {
				...
				for (Record rec : getRecords(id)) {
					...
					try {
						...
						throw new IllegalStateException(format("Erro ao tentar escrever o registro \"%s\".", rec.getName()), e);
					}
