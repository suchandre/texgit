...
class FixedFieldBuilder {
	...
	private static FixedField<?> getInstance(MetaField metaField) throws ParseException {
		...
		case BIGDECIMAL:
			...
			if (StringUtils.isNotBlank(metaField.getValue()))
				fBDL.setValue(new BigDecimal(DecimalFormat.class.cast(formatter).parse(metaField.getValue()).doubleValue()));
			else
		...
		case DATE:
			...
			if (StringUtils.isNotBlank(metaField.getValue())) {
				...
				fDTE.setValue(DateFormat.class.cast(formatter).parse(metaField.getValue()));
			} else {
	...
	private static Format getFormater(MetaField metaField) {
		...
		if (isNotNull(metaField.getFormatter())) {
			...
			formatter = buildFormat(metaField.getFormatter().getFormat(), metaField.getFormatter().getType());
	...
	private static String buildFormat(EnumFormats format, EnumFormatsTypes type) {
		...
		case DECIMAL:
			...
			int lengthToFill = BASE_DECIMAL_FORMAT.length() + StringUtils.countMatches(defFormat, "D");
